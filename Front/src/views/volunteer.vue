<template>
	<el-button type="primary" :icon="CirclePlus" circle :disabled="role != '基层支部书记'" @click="addMes"
		style="position: absolute;right:10px;z-index: 10;" />
	<el-table :data="tableData" style="width: 100%">
		<el-table-column label="志愿者" width="130">
			<template #default="scope">
				<p v-show="modifyTable[scope.$index] == 1">{{ scope.row.user }}</p>
				<el-input v-show="modifyTable[scope.$index] != 1" v-model="scope.row.user"></el-input>
			</template>
		</el-table-column>
		...
		<el-table-column label="党员号" width="200">
			<template #default="scope">
				<p v-show="modifyTable[scope.$index] == 1">{{ scope.row.partyid }}</p>
				<el-input v-show="modifyTable[scope.$index] != 1" v-model="scope.row.partyid"></el-input>
			</template>
		</el-table-column>
		<el-table-column label="志愿地址" width="120">
			<template #default="scope">
				<p v-show="modifyTable[scope.$index] == 1">{{ scope.row.volunteeraddress }}</p>
				<el-input v-show="modifyTable[scope.$index] != 1" v-model="scope.row.volunteeraddress"></el-input>
			</template>
		</el-table-column>
		<el-table-column label="志愿标题" width="120">
			<template #default="scope">
				<p v-show="modifyTable[scope.$index] == 1">{{ scope.row.volunteertitle }}</p>
				<el-input v-show="modifyTable[scope.$index] != 1" v-model="scope.row.volunteertitle"></el-input>
			</template>
		</el-table-column>
		...
		<el-table-column label="开始时间" width="200">
			<template #default="scope">
				...
				<div class="block" v-show="modifyTable[scope.$index] != 1">
					...
					<el-date-picker v-model="scope.row.starttime" type="date" placeholder="Pick a day"
		...
		<el-table-column label="结束时间" width="200">
			<template #default="scope">
				...
				<div class="block" v-show="modifyTable[scope.$index] != 1">
					...
					<el-date-picker v-model="scope.row.endtime" type="date" placeholder="Pick a day"
						:disabled-date="disabledDate" :shortcuts="shortcuts" :size="size" />
				</div>
			</template>
		</el-table-column>
		<el-table-column fixed="right" label="操作" width="120">
			<template #default="scope">
				...
				<el-button link type="primary" size="small" @click="deleteClick(scope)"
					:disabled="role != '基层支部书记'">删除</el-button>
			</template>
		</el-table-column>
	</el-table>
	<div style="position: absolute;bottom: 10px;left:45%">
		<el-pagination v-model:current-page="changePage.currentPage" background :total="changePage.total"
			@current-change="currentChange" layout="prev, pager, next" />
	</div>
</template>

<script setup>
import { findVolunteerList, addVolunteer, updateVolunteer, deleteVolunteer } from '../api/VolunteerAPI.js'
...
const handleClick = (scope, index) => {
	if (index == 1) {
		...
		updateVolunteer(scope.row).then(res => {
			if (res.code == 200) {
				getList()
				ElMessage({
					message: '添加成功',
		...
		scope.row.starttime = new Date(scope.row.starttime).toISOString()
		scope.row.endtime = new Date(scope.row.endtime).toISOString()
		addVolunteer(scope.row).then(res => {
			if (res.code == 200) {
				getList()
				ElMessage({
					message: res.data,
...
const deleteClick = (scope) => {
	if (modifyTable.value[scope.$index] == 3) {
		...
		deleteVolunteer({ id: scope.row.id }).then(res => {
			if (res.code == 200) {
				modifyTable.value.splice(scope.$index, 1)
				tableData.value.splice(scope.$index, 1)
				ElMessage({
...
const getList = () => {
	findVolunteerList({ belongto: JSON.parse(localStorage.getItem('token')).belongto }, { page: changePage.currentPage, size: 10 }).then(res => {
		if (res.code == 200) {
